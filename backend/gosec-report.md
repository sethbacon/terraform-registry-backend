...
**Previous scan:** 2026-02-24 (local full scan)
**Tool:** [gosec](https://github.com/securego/gosec) `@latest` (CI) / `dev` (local)
**Scope:** `./...` (all Go packages)
**Filters:** none
**Files scanned:** 122 | **Lines scanned:** 35,598 | **nosec suppressions:** 76

---

## Summary

| Severity | 2026-02-17 | 2026-02-19 | 2026-02-23 (CI) | 2026-02-23 (local) | 2026-02-24 (local) | 2026-03-01 (CI) |
| --- | --- | --- | --- | --- | --- | --- |
...
>
> **2026-03-01:** CI now uses a newer `@latest` gosec that includes rules G118, G122, and additional G706
> sites not present in the local `dev` build. All 8 new CI findings are false positives — `#nosec` annotations
> added; baseline regenerated. See New Findings (2026-03-01) below.

### Rule-level breakdown

| Rule | 2026-02-23 (local) | 2026-02-24 raw | After fixes | 2026-03-01 CI new | Status |
| --- | --- | --- | --- | --- | --- |
| G101 | 0 ✅ | 0 ✅ | 0 ✅ | 0 | Fixed (nosec) ✅ |
| G108 | 0 ✅ | 0 ✅ | 0 ✅ | 0 | Previously fixed ✅ |
| G114 | 0 ✅ | 0 ✅ | 0 ✅ | 0 | Previously fixed ✅ |
| G115 | 2 | 2 | 2 | 0 | False positive |
| G117 | 22 | 23 | **22** ✅ | 0 | Fixed (nosec) ✅ |
| G118 | — | — | — | **2** | **Fixed (nosec)** ✅ |
| G122 | — | — | — | **1** | **Fixed (nosec)** ✅ |
| G201 | 3 | 3 | 3 | 0 | False positive |
| G202 | 1 | 1 | 1 | 0 | False positive |
| G304 | 8 | 8 | 8 | 0 | Accepted risk |
| G305 | 1 | 1 | 1 | 0 | False positive |
| G701 | 4 | 9 | **11** | 0 | False positive (baseline updated) |
| G703 | 0 ✅ | 0 ✅ | 0 ✅ | 0 | Previously fixed ✅ |
| G704 | 41 | 41 | 41 | 0 | Accepted risk |
| G706 | 12 | 12 | 12 | **4** | **Fixed (nosec)** ✅ |

1 net-new finding (G117 ×1) fixed immediately; G701 count is now 11 (+7 vs 2026-02-23) from stats.go expansion — all false positives, baseline updated — see New Findings (2026-02-24) below.

---

## New Findings (2026-03-01 — CI `@latest` gosec, new rules)

CI started using a newer gosec release that introduced rules G118, G122, and flagged 4 additional G706
sites not present in the local `dev` build. All 8 findings are false positives; `#nosec` annotations
added and baseline regenerated.

### G118 MEDIUM/HIGH — Context cancellation not called / goroutine uses Background context (×2) — **FALSE POSITIVE, nosec added**

**Finding 1:** `internal/api/webhooks/scm_webhook.go:178`

```go
asyncCtx, asyncCancel := context.WithTimeout(context.Background(), 10*time.Minute)
```

gosec reports `asyncCancel` is not called. It is called via `defer asyncCancel()` inside the `safego.Go`
goroutine closure immediately below. gosec does not track the cancel through the closure.

**Finding 2:** `internal/jobs/mirror_sync.go:1041`

```go
go j.syncMirror(context.Background(), *config)
```

An existing comment already explains: the HTTP request context is cancelled when the response is sent;
background context is intentional for the async sync operation.

**Action:** `// #nosec G118` added to both lines with justification comments. No code change required.

---

### G122 HIGH/MEDIUM — Filesystem operation in WalkDir callback uses race-prone path (×1) — **FALSE POSITIVE, nosec added**

**File:** `internal/services/scm_publisher.go:296`

```go
file, err := os.Open(path) // previously: #nosec G304
```

The line already had `#nosec G304` for the file-inclusion rule. G122 is a new rule flagging `filepath.Walk`
callback paths for TOCTOU symlink traversal. The path comes from `filepath.WalkDir` over an extracted
archive that has already been validated against path traversal (see G305 entry). No symlink TOCTOU risk.

**Action:** Expanded annotation to `#nosec G304 G122` with existing justification. No code change required.

---

### G706 LOW/HIGH — Log injection via taint analysis (×4 new sites) — **FALSE POSITIVE, nosec added**

**File:** `cmd/server/main.go` lines 217, 218, 219, 225
...
All four values are operator-supplied configuration (address, base URL, storage backend name, TLS cert path).
They come from `config.yaml` / environment variables — not from HTTP request input. Log injection is only
exploitable when the attacker controls the logged value; operators who control the config already have full
server access.

**Action:** `// #nosec G706` added to lines 217, 218, 219, 225 with justification. No code change required.
