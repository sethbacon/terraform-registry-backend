...
type publicRouteDeps struct {
	...
	platformAdminCarrier    *platformadmin.Carrier
...
func registerPublicRoutes(router *gin.Engine, d *publicRouteDeps) {
	...
	platformAdminCarrier := d.platformAdminCarrier
...
<html>
	...
	v1Modules.Use(middleware.OptionalAuthMiddleware(cfg, userRepo, apiKeyRepo, orgRepo, tokenRepo, userTokenRevocationRepo, platformAdminCarrier))
	{
	...
	v1Providers.Use(middleware.OptionalAuthMiddleware(cfg, userRepo, apiKeyRepo, orgRepo, tokenRepo, userTokenRevocationRepo, platformAdminCarrier))
	{
...
func registerAuthenticatedGroupMiddleware(authenticatedGroup *gin.RouterGroup, d *apiV1RouteDeps) {
	authenticatedGroup.Use(middleware.AuthMiddleware(d.cfg, d.userRepo, d.apiKeyRepo, d.orgRepo, d.tokenRepo, d.userTokenRevocationRepo, d.platformAdminCarrier))
	...
	apiV1 := router.Group("/api/v1")
	{
		...
		authGroup.Use(middleware.RateLimitMiddleware(authRateLimiter))
		{
			...
			authGroup.POST("/logout",
				...
				middleware.OptionalAuthMiddleware(cfg, userRepo, apiKeyRepo, orgRepo, tokenRepo, userTokenRevocationRepo, platformAdminCarrier),
		...
		publicDetailGroup.Use(middleware.OptionalAuthMiddleware(cfg, userRepo, apiKeyRepo, orgRepo, tokenRepo, userTokenRevocationRepo, platformAdminCarrier))
		...
		if admin.IsDevMode() {
			...
			devGroup.Use(admin.DevModeMiddleware())
			{
				...
				devGroup.Use(middleware.AuthMiddleware(cfg, userRepo, apiKeyRepo, orgRepo, tokenRepo, userTokenRevocationRepo, platformAdminCarrier))
...
func registerSCIMRoutes(router *gin.Engine, d *apiV1RouteDeps) {
	...
	scimGroup.Use(middleware.AuthMiddleware(d.cfg, d.userRepo, d.apiKeyRepo, d.orgRepo, d.tokenRepo, d.userTokenRevocationRepo, d.platformAdminCarrier))
