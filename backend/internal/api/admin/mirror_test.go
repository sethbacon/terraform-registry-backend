...
func TestMirrorCreate_WithUserIDContext(t *testing.T) {
	...
	sqlxDB := sqlx.NewDb(db, "sqlmock")
	mirrorRepo := repositories.NewMirrorRepository(sqlxDB)
	orgRepo2 := repositories.NewOrganizationRepository(db)
...
var mirroredProviderVersionCols = []string{
	"id", "mirrored_provider_id", "provider_version_id",
	"upstream_version", "synced_at", "shasum_verified", "gpg_verified",
}
...
func newMirrorProvidersRouter(t *testing.T) (sqlmock.Sqlmock, *gin.Engine) {
	...
	sqlxDB := sqlx.NewDb(db, "sqlmock")
	h := NewMirrorHandler(
		repositories.NewMirrorRepository(sqlxDB),
		repositories.NewOrganizationRepository(db),
		repositories.NewProviderRepository(db),
	)
	r := gin.New()
	r.GET("/mirrors/:id/providers", h.ListMirroredProviders)
	return mock, r
}

func TestListMirroredProviders_InvalidID(t *testing.T) {
	_, r := newMirrorProvidersRouter(t)
	w := httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest("GET", "/mirrors/not-a-uuid/providers", nil))
	if w.Code != http.StatusBadRequest {
		t.Errorf("status = %d, want 400", w.Code)
	}
}
...
func TestListMirroredProviders_GetByIDDBError(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest("GET", "/mirrors/"+knownUUID+"/providers", nil))
	if w.Code != http.StatusInternalServerError {
		t.Errorf("status = %d, want 500", w.Code)
	}
}

func TestListMirroredProviders_NotFound(t *testing.T) {
	mock, r := newMirrorProvidersRouter(t)
	mock.ExpectQuery("SELECT.*FROM mirror_configurations WHERE id").
		WillReturnRows(sqlmock.NewRows(mirrorCfgCols))
	w := httptest.NewRecorder()
...
func TestListMirroredProviders_ListProvidersDBError(t *testing.T) {
	mock, r := newMirrorProvidersRouter(t)
	mock.ExpectQuery("SELECT.*FROM mirror_configurations WHERE id").
		WillReturnRows(sampleMirrorCfgRow())
...
func TestListMirroredProviders_EmptyProviders(t *testing.T) {
	mock, r := newMirrorProvidersRouter(t)
	mock.ExpectQuery("SELECT.*FROM mirror_configurations WHERE id").
		WillReturnRows(sampleMirrorCfgRow())
	mock.ExpectQuery("SELECT.*FROM mirrored_providers").
...
func TestListMirroredProviders_WithProviderAndVersions(t *testing.T) {
	mock, r := newMirrorProvidersRouter(t)
	now := time.Now()
	providerID := uuid.MustParse(knownUUID)
	versionID := uuid.MustParse(knownUUID)
	...
	mock.ExpectQuery("SELECT.*FROM mirrored_provider_versions").
		WillReturnRows(sqlmock.NewRows(mirroredProviderVersionCols).AddRow(
			knownUUID, knownUUID, versionID, "1.0.0", now, true, true,
		))
	mock.ExpectQuery("SELECT.*FROM provider_platforms").
		WillReturnRows(sqlmock.NewRows(providerPlatformCols).AddRow(
...
func TestMirrorRegisterRoutes(t *testing.T) {
	...
	for _, route := range r.Routes() {
		pathSet[route.Method+":"+route.Path] = true
	}
...
// ---------------------------------------------------------------------------
// GetMirrorStatus — additional error paths
// ---------------------------------------------------------------------------

func TestMirrorGetStatus_GetByIDDBError(t *testing.T) {
	mock, r := newMirrorRouter(t)
	mock.ExpectQuery("SELECT.*FROM mirror_configurations WHERE id").
		WillReturnError(fmt.Errorf("db error"))
	w := httptest.NewRecorder()
...
func TestMirrorGetStatus_ActiveSyncDBError(t *testing.T) {
	mock, r := newMirrorRouter(t)
	mock.ExpectQuery("SELECT.*FROM mirror_configurations WHERE id").
		WillReturnRows(sampleMirrorCfgRow())
	mock.ExpectQuery("SELECT.*FROM mirror_sync_history WHERE mirror_config_id.*AND status").
...
func TestMirrorGetStatus_SyncHistoryDBError(t *testing.T) {
	mock, r := newMirrorRouter(t)
	mock.ExpectQuery("SELECT.*FROM mirror_configurations WHERE id").
		WillReturnRows(sampleMirrorCfgRow())
	mock.ExpectQuery("SELECT.*FROM mirror_sync_history WHERE mirror_config_id.*AND status").
...
func TestMirrorGetStatus_WithNextScheduled(t *testing.T) {
	mock, r := newMirrorRouter(t)
	// Row where enabled=true and last_sync_at is set — triggers nextScheduled calculation.
	lastSync := time.Now().Add(-2 * time.Hour)
	syncedRow := sqlmock.NewRows(mirrorCfgCols).AddRow(
		knownUUID, "test-mirror", nil, "https://registry.terraform.io", nil,
		nil, nil, nil, nil,
		true, 24, &lastSync, nil, nil,
		time.Now(), time.Now(), nil,
	)
	mock.ExpectQuery("SELECT.*FROM mirror_configurations WHERE id").
		WillReturnRows(syncedRow)
	mock.ExpectQuery("SELECT.*FROM mirror_sync_history WHERE mirror_config_id.*AND status").
		WillReturnRows(emptySyncHistRows())
	mock.ExpectQuery("SELECT.*FROM mirror_sync_history WHERE mirror_config_id").
		WillReturnRows(emptySyncHistRows())
...
// ---------------------------------------------------------------------------
// TriggerSync — additional error paths
// ---------------------------------------------------------------------------

func TestMirrorTriggerSync_GetByIDDBError(t *testing.T) {
	mock, r := newMirrorRouter(t)
	mock.ExpectQuery("SELECT.*FROM mirror_configurations WHERE id").
		WillReturnError(fmt.Errorf("db error"))
	w := httptest.NewRecorder()
...
// ---------------------------------------------------------------------------
// TriggerSync — syncJob returns unexpected error (not "already in progress")
// ---------------------------------------------------------------------------

func TestMirrorTriggerSync_JobError(t *testing.T) {
	mock, r := newMirrorRouterWithJob(t, &mockSyncJob{err: fmt.Errorf("unexpected failure")})
	mock.ExpectQuery("SELECT.*FROM mirror_configurations WHERE id").
		WillReturnRows(sampleMirrorCfgRow())
	w := httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest("POST", "/mirrors/"+knownUUID+"/sync", nil))
