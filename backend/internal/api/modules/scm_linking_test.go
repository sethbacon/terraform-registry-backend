...
// This is the regression test for the nil-pointer panic introduced when
// provider.BaseURL was dereferenced unconditionally.
func TestLinkModule_Success_NilBaseURL(t *testing.T) {
	scmMock, modMock, r := newSCMLinkingRouter(t)

	// Module exists
	modMock.ExpectQuery("SELECT.*FROM modules m.*WHERE m.id").
		WillReturnRows(sampleModuleForSCMRow(scmLinkModuleUUID))
	// Provider exists with nil base_url (sampleSCMProviderRowLink already has base_url=nil)
	scmMock.ExpectQuery("SELECT.*FROM scm_providers WHERE id").
		WillReturnRows(sampleSCMProviderRowLink())
	...
	// Response must contain the webhook callback URL
...
func TestGetUserIDFromContext_Missing(t *testing.T) {
	...
	if err == nil {
		t.Error("want error when user_id missing from context")
	}
}

func TestGetUserIDFromContext_UUIDType(t *testing.T) {
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	id := uuid.New()
	c.Set("user_id", id)
...
func TestGetUserIDFromContext_StringType(t *testing.T) {
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	id := uuid.New()
...
func TestGetUserIDFromContext_InvalidString(t *testing.T) {
	...
	_, err := getUserIDFromContext(c)
	if err == nil {
		t.Error("want error for invalid UUID string")
	}
}

func TestGetUserIDFromContext_UnexpectedType(t *testing.T) {
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Set("user_id", 12345)
	_, err := getUserIDFromContext(c)
	if err == nil {
