...
| 000057 | `legal_holds`                          | ⚠️ Destructive | Creates `legal_holds`, the table the audit retention sweep consults before deleting (issue #872), replacing the startup `EnsureTable` that created it outside the migration chain (#864's class). The shape is rendered by `store.LegalHoldTableDDL` in terraform-suite-identity — the package whose `NOT EXISTS` reads it — and `TestMigration000057MatchesTheLibraryDDL` fails if the two drift. The down migration is a real `DROP`, and it is destructive in a way the up is not: it removes the record of which audit entries an investigation asked to be preserved, and the next sweep then deletes them. Back up any active hold first; the `.down.sql` carries the `COPY` statement |
| 000058 | `audit_logs_actor_email`                | ⚠️ Lossy down  | Adds `actor_email` to this chain's `audit_logs`, the column the shared identity store has written and read since v0.25.0 (issue #864). Both its statements name the column unqualified, so with `TFR_IDENTITY_SCHEMA_ENABLED` off — the default — they resolve to `public.audit_logs` and every audited request and every audit-log read died on 42703. Enabling the identity migrations does NOT fix it: identity's 000007 is schema-qualified and adds the column to a table the default configuration never touches. Additive and inert after a later cutover. The down migration DROPs the column, discarding retained attribution for rows whose user has since been deleted — the case the column exists for; the `.down.sql` carries the `COPY` to preserve it |
| 000059 | `group_mappings`                       | ✅ Yes         | Drops `group_mappings`, registry's own per-app IdP group → role-template mapping table (terraform-suite-identity#206 phase 2, same phase as 000055). Safe while nothing reads it: every row is derived — dual-written beside the authoritative write to `oidc_config.extra_config` or reconciled from it at startup — so the rollback changes no login-time group resolution. The up migration creates the table EMPTY and deliberately performs no SQL backfill: the source rows live in `oidc_config` on the identity connection, whose schema and database are chosen at process start, so the backfill runs in Go at startup instead (`ReconcileGroupMappings`). Stops being safe once the read cutover ships; roll the binary back first |
| 000060 | `notify_dedup_claims`                  | ✅ Yes         | Same class as 000058 (issue #864), caught before shipping this time: `identity/store.NotifyDedupRepository.ClaimDedup` (terraform-suite-identity#157) names `notify_dedup_claims` unqualified, so with `TFR_IDENTITY_SCHEMA_ENABLED` off — the default — it resolves to `public`, which identity migration 000008 never touches without `TFR_IDENTITY_MIGRATIONS_ENABLED` also on. Without this migration every scanner-update notification's dedup claim would 42P01, silently no-op'd only by `claimDedup`'s fail-open handling — the dedup guarantee just wouldn't apply, in the default deployment shape, forever. Additive and inert after a later cutover; the table holds only short-lived reservations, so the rollback discards nothing of lasting value |
| 000061 | `backfill_null_organization`           | ⚠️ Lossy down  | Assigns every `providers`/`modules` row with a NULL `organization_id` to the organization named by `TFR_BACKFILL_ORGANIZATION_ID`, then makes the column `NOT NULL` (issue #1035). Aborts with instructions rather than guessing when NULL rows exist and no organization is named, and aborts naming the conflicts when a backfill would collide with a row the target organization already owns — collapsing several NULL rows onto one owner is a merge, which is an operator decision. The down migration drops `NOT NULL` only: once a NULL has been replaced with a real owner nothing records which rows held one, so the backfill itself is not reversible. |
| 000062 | `entra_credential_type`                | ✅ Yes         | Adds `entra_credential_type` to `scm_providers` (`client_secret` | `federated`, default `client_secret`) so an `entra_app` provider can mint its token from a workload-identity projected token instead of a stored client secret (issue #1037). A partial CHECK, `scm_providers_entra_credential_shape`, holds the two shapes apart: `client_secret` requires a stored secret, `federated` forbids one — the column alone would let a row claim federation while still carrying the secret it was supposed to retire. Down drops both the CHECK and the column; any provider that had been switched to `federated` reverts to `client_secret` with no secret to mint from and must be re-credentialed. Numbered 000062 because 000061 belongs to the organization backfill (issue #1035). |
| 000063 | `entra_certificate`                    | ⚠️ Guarded down | Adds `encrypted_entra_certificate` to `scm_providers` and widens `entra_credential_type` to `client_secret` | `federated` | `certificate` (issue #1041): an `entra_app` provider may prove itself by signing a client assertion with a held key, so no secret ever crosses the wire and the credential's lifetime is the operator's rather than Entra's 24-month cap. The column holds the whole PEM bundle -- certificate plus unencrypted key -- sealed with the token cipher and bound to the row, exactly as the GitHub App key is. `scm_providers_entra_credential_shape` is re-stated so each type carries exactly its own material. Drops 000062's inline CHECK by catalog lookup rather than by an assumed auto-generated name. **Down refuses** while any provider uses a certificate: under the two-value schema such a row could not mint and would fail 000062's shape CHECK, so the operator re-credentials first (000061's fail-closed posture). |
| 000064 | `entra_managed_identity`               | ⚠️ Guarded down | Adds `managed_identity` to `entra_credential_type` and a fourth arm to `scm_providers_entra_credential_shape` (issue #1042): the hosting platform holds the credential, so the row carries only `client_id` -- no secret, no certificate, and **no new column**. Whether a deployment can mint one is NOT a schema question: managed identity works only on Azure compute, so availability is declared by the operator in `TFR_SCM_ENTRA_CREDENTIAL_TYPES` and enforced in the handler; the CHECK only keeps the row's shape honest. Drops 000063's type CHECK by catalog lookup on the `= ANY` form. **Down refuses** while any provider uses a managed identity. |
