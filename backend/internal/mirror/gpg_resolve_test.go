package mirror

import "testing"

// expiredHashiCorpKey is the pre-2026-04-18 HashiCorp releases GPG key that
// expired. Used to test that ResolveExpiredGPGKey substitutes the refreshed
// snapshot for expired keys with the known fingerprint.
const expiredHashiCorpKey = `-----BEGIN PGP PUBLIC KEY BLOCK-----


=7pIB
-----END PGP PUBLIC KEY BLOCK-----`

func TestResolveExpiredGPGKey_ValidKey_ReturnsUnchanged(t *testing.T) {
	result := ResolveExpiredGPGKey(HashiCorpReleasesGPGKey)
	if result != HashiCorpReleasesGPGKey {
		t.Error("expected valid key to be returned unchanged")
	}
}

func TestResolveExpiredGPGKey_ExpiredHashiCorpKey_Substituted(t *testing.T) {
	result := ResolveExpiredGPGKey(expiredHashiCorpKey)
	if result == expiredHashiCorpKey {
		t.Fatal("expected expired key to be substituted")
	}
	if result != HashiCorpReleasesGPGKey {
		t.Error("expected substitution to return the current embedded key")
	}
}

func TestResolveExpiredGPGKey_EmptyString_ReturnsUnchanged(t *testing.T) {
	result := ResolveExpiredGPGKey("")
	if result != "" {
		t.Error("expected empty string to be returned unchanged")
	}
}

func TestResolveExpiredGPGKey_Garbage_ReturnsUnchanged(t *testing.T) {
	garbage := "not a real PGP key"
	result := ResolveExpiredGPGKey(garbage)
	if result != garbage {
		t.Error("expected unparseable input to be returned unchanged")
	}
}

func TestResolveExpiredGPGKey_UnknownExpiredKey_ReturnsUnchanged(t *testing.T) {
	result := ResolveExpiredGPGKey(OpenTofuReleasesGPGKey)
	if result != OpenTofuReleasesGPGKey {
		t.Error("expected non-HashiCorp key to be returned unchanged")
	}
}

func TestHasUsableGPGKey_ValidKey(t *testing.T) {
	if !HasUsableGPGKey(HashiCorpReleasesGPGKey) {
		t.Error("expected current HashiCorp key to be usable")
	}
}

func TestHasUsableGPGKey_ExpiredKey(t *testing.T) {
	if HasUsableGPGKey(expiredHashiCorpKey) {
		t.Error("expected expired key to be not usable")
	}
}

func TestHasUsableGPGKey_Empty(t *testing.T) {
	if HasUsableGPGKey("") {
		t.Error("expected empty string to be not usable")
	}
}
