...
// newTestPublisher returns an SCMPublisher that writes temp files inside
// t.TempDir() so they are cleaned up automatically after each test.
func newTestPublisher(t *testing.T) *SCMPublisher {
	t.Helper()
	return &SCMPublisher{tempDir: t.TempDir()}
}
...
// ---------------------------------------------------------------------------
// downloadAndPackage — wrapper-directory path resolution
// ---------------------------------------------------------------------------

// mockConnector implements scm.Connector for testing.
// Only DownloadSourceArchive does something meaningful; all other methods are stubs.
type mockConnector struct {
	archiveData []byte
	archiveErr  error
}

func (m *mockConnector) Platform() scm.ProviderKind                    { return scm.ProviderGitHub }
func (m *mockConnector) AuthorizationEndpoint(string, []string) string { return "" }
func (m *mockConnector) CompleteAuthorization(context.Context, string) (*scm.AccessToken, error) {
	return nil, nil
}
func (m *mockConnector) RenewToken(context.Context, string) (*scm.AccessToken, error) {
	return nil, nil
}
func (m *mockConnector) FetchRepositories(context.Context, *scm.AccessToken, scm.Pagination) (*scm.RepoListResult, error) {
	return nil, nil
}
func (m *mockConnector) FetchRepository(context.Context, *scm.AccessToken, string, string) (*scm.SourceRepo, error) {
	return nil, nil
}
func (m *mockConnector) SearchRepositories(context.Context, *scm.AccessToken, string, scm.Pagination) (*scm.RepoListResult, error) {
	return nil, nil
}
func (m *mockConnector) FetchBranches(context.Context, *scm.AccessToken, string, string, scm.Pagination) ([]*scm.GitBranch, error) {
	return nil, nil
}
func (m *mockConnector) FetchTags(context.Context, *scm.AccessToken, string, string, scm.Pagination) ([]*scm.GitTag, error) {
	return nil, nil
}
func (m *mockConnector) FetchTagByName(context.Context, *scm.AccessToken, string, string, string) (*scm.GitTag, error) {
	return nil, nil
}
func (m *mockConnector) FetchCommit(context.Context, *scm.AccessToken, string, string, string) (*scm.GitCommit, error) {
	return nil, nil
}
func (m *mockConnector) DownloadSourceArchive(_ context.Context, _ *scm.AccessToken, _, _, _ string, _ scm.ArchiveKind) (io.ReadCloser, error) {
	if m.archiveErr != nil {
		return nil, m.archiveErr
	}
	return io.NopCloser(bytes.NewReader(m.archiveData)), nil
}
func (m *mockConnector) RegisterWebhook(context.Context, *scm.AccessToken, string, string, scm.WebhookSetup) (*scm.WebhookInfo, error) {
	return nil, nil
}
func (m *mockConnector) RemoveWebhook(context.Context, *scm.AccessToken, string, string, string) error {
	return nil
}
func (m *mockConnector) ParseDelivery([]byte, map[string]string) (*scm.IncomingHook, error) {
	return nil, nil
}
func (m *mockConnector) VerifyDeliverySignature([]byte, string, string) bool { return true }

// makeGitHubTarGz wraps files inside a single root directory, mimicking
// the archive format returned by GitHub's tarball download endpoint.
func makeGitHubTarGz(t *testing.T, wrapperDir string, files map[string]string) []byte {
	t.Helper()
	prefixed := make(map[string]string, len(files))
	for name, content := range files {
		prefixed[wrapperDir+"/"+name] = content
	}
	return makeTarGz(t, prefixed)
}

// TestDownloadAndPackage_GitHubWrapperWithSlashSubpath verifies that when
// subpath is "/" (the default) and the archive uses a GitHub-style single
// wrapper directory, the .tf files at the repo root are found correctly.
// This is the primary regression test for the path-resolution bug fixed in
// this patch (scm_publisher.go: wrapper dir detection fired only on IsNotExist,
// but filepath.Join(tempDir, "/") always resolves to tempDir itself).
func TestDownloadAndPackage_GitHubWrapperWithSlashSubpath(t *testing.T) {
	archiveData := makeGitHubTarGz(t, "terraform-azurerm-vm-abc123", map[string]string{
		"main.tf":      `resource "azurerm_virtual_machine" "vm" {}`,
		"variables.tf": `variable "vm_size" {}`,
	})

	p := newTestPublisher(t)
	connector := &mockConnector{archiveData: archiveData}

	outputPath, checksum, err := p.downloadAndPackage(context.Background(), connector, nil,
		"sethbacon", "terraform-azurerm-vm", "abc123", "/")
	if err != nil {
	...
	if _, err := os.Stat(outputPath); err != nil {
		t.Errorf("output tarball not found: %v", err)
	}
}

// TestDownloadAndPackage_GitHubWrapperWithExplicitSubpath verifies that a
// module living in a subdirectory of the repo (e.g. "modules/vm") is
// resolved correctly relative to the unwrapped archive root.
func TestDownloadAndPackage_GitHubWrapperWithExplicitSubpath(t *testing.T) {
	archiveData := makeGitHubTarGz(t, "monorepo-abc123", map[string]string{
		"modules/vm/main.tf":      `resource "azurerm_virtual_machine" "vm" {}`,
		"modules/vm/variables.tf": `variable "vm_size" {}`,
		"README.md":               "# monorepo",
	})
	...
	_, _, err := p.downloadAndPackage(context.Background(), connector, nil,
		"owner", "monorepo", "abc123", "modules/vm")
	if err != nil {
		t.Fatalf("unexpected error for explicit subpath: %v", err)
	}
}
...
func TestDownloadAndPackage_SubpathNotFound(t *testing.T) {
	...
	_, _, err := p.downloadAndPackage(context.Background(), connector, nil,
		"owner", "repo", "abc123", "nonexistent/path")
	if err == nil {
		t.Fatal("expected error for missing subpath, got nil")
	}
}

// TestDownloadAndPackage_NoTFFilesAtRoot verifies that a repository with no
// .tf files at the configured module path fails with a validation error.
func TestDownloadAndPackage_NoTFFilesAtRoot(t *testing.T) {
	archiveData := makeGitHubTarGz(t, "repo-abc123", map[string]string{
		"README.md": "# My module",
	})
	...
	_, _, err := p.downloadAndPackage(context.Background(), connector, nil,
		"owner", "repo", "abc123", "/")
	if err == nil {
		t.Fatal("expected validation error for missing .tf files, got nil")
	}
}

// TestDownloadAndPackage_DownloadError verifies that a connector-level download
// failure is propagated as an error.
func TestDownloadAndPackage_DownloadError(t *testing.T) {
	p := newTestPublisher(t)
	connector := &mockConnector{archiveErr: errors.New("network failure")}
