...
| I-8 | Outbound "Consumed by" proxy call in `suite.go` (TB-8) reaches an operator-configured sibling URL | Backend suite proxy | Routed through the centralized `httpsafe` resolve-and-pin SSRF guard (`suite.go`, `httpsafe.NewClient`), like every other outbound call to an operator-configured URL. Destination host is additionally fixed to the discovery-advertised sibling `PublicURL` (built via `net/url`, not string concatenation); user/module input is confined to query parameters | ✅ Implemented |
