...
| `TFR_SUITE_ROLE_SEED_OWNER`       | string   | `self`  | Which app seeds the **shared** identity role templates: `self`, `registry`, or `tsm`. With a shared identity DB, exactly one app must own the seed. Registry no longer *reads* that table — see the note below — but the flag is still load-bearing. **With `TFR_SUITE_IDENTITY_SHARED_STORE=true` the server refuses to start while this is `self`**, and the value must be the SAME in both deployments — naming a different owner in each leaves both seeding, which no process can detect. |
| `TFR_SUITE_IDENTITY_SHARED_STORE` | bool     | `false` | Operator assertion that this app uses the shared identity store + single IdP (advertised in the manifest). **Requires `TFR_IDENTITY_SCHEMA_ENABLED=true`; the server refuses to start otherwise**, since with the cutover off identity resolves from this app's own `public` schema and no store is shared. The reverse is fine: a standalone registry may run the cutover without asserting this. |
